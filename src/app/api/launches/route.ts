...
/** A profile coin launched through Kore (always Pons). */
...
/** GET /api/launches?limit=48 - newest profile launches (from KV). */
export async function GET(req: Request) {
  ...
  const deployer = searchParams.get("deployer"); // optional: filter by creator
  ...
  // Single-token lookup (used as an image/metadata fallback on the profile page).
  ...
  const items = all
    .filter((r) => (deployer && isAddress(deployer) ? r.deployer.toLowerCase() === deployer.toLowerCase() : true))
    .slice(0, limit);

  return NextResponse.json({ items });
}

/** POST /api/launches - record a profile launch made through Kore. */
export async function POST(req: Request) {
  ...
  const record: LaunchRecord = {
    ...
    deployer:
      body.deployer && isAddress(body.deployer)
        ? body.deployer
...
function removeMessage(token: string): string {
  return `Remove ${token.toLowerCase()} from the Kore feed`;
}
 ...
 * removeMessage(token) recovers to the record's deployer.
 */
