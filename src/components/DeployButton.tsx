 ...
 * signs writeContract, then we parse the receipt for the new token address and
 * record it to the profile feed.
 */
export function DeployButton({
    ...
    return (
      <div className="space-y-2">
        ...
        <p className="text-xs text-zinc-500">
          This app runs on {robinhoodChain.name} (an EVM chain, not Solana). Switch to launch.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-2">
      {warnings.length > 0 && (
        <ul className="space-y-1 rounded-xl border border-ember/30 bg-ember/[0.06] p-3 text-xs text-pink">
          {warnings.map((w, i) => (
            <li key={i}>⚠ {w}</li>
          ))}
        </ul>
      ...
      {!ready && <p className="text-xs text-zinc-500">Launch is currently unavailable.</p>}
      {!isConnected && ready && <p className="text-xs text-zinc-500">Connect your wallet to launch.</p>}
