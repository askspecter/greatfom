...
// Acceptance is stored per wallet address (like Pons), so the gate reappears for
// each new wallet.
const keyFor = (address: string) => `fomo:accepted:v1:${address.toLowerCase()}`;

/**
 * First-time connect gate: the first time a given wallet connects, the user must
 * accept the Terms of Use and Privacy Policy before using the app. Remembered
 * per wallet address, so it only appears once per wallet.
 */
